...
public class RoomService {
	...
	public List<RoomModel> getAllRooms() {
		...
		String sql = "SELECT r.*, p.*, l.* FROM room r " + "JOIN price p ON r.price_ID = p.price_ID "
		...
		try (Connection conn = DbConfig.getDbConnection();
			...
			while (rs.next()) {
				PriceModel price = new PriceModel(rs.getInt("price_ID"), rs.getInt("price_monthly"),
						rs.getInt("price_deposit_amount"));

				LocationModel location = new LocationModel(rs.getInt("location_ID"), rs.getString("location_city"),
						rs.getString("location_area"));

				RoomModel room = new RoomModel(rs.getInt("room_ID"), rs.getString("room_type"),
						rs.getString("room_description"), rs.getString("room_status"), rs.getInt("room_capacity"),
						rs.getString("room_furnishing"), rs.getString("room_facilities"), rs.getInt("room_size"),
						rs.getString("image_path"), price, location);
				rooms.add(room);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<LocationModel> getAllLocations() {
		...
		try (Connection conn = DbConfig.getDbConnection();
				...
				ResultSet rs = stmt.executeQuery()) {
			while (rs.next()) {
				list.add(new LocationModel(rs.getInt("location_ID"), rs.getString("location_city"),
						rs.getString("location_area")));
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<PriceModel> getAllPrices() {
		...
		try (Connection conn = DbConfig.getDbConnection();
				...
				ResultSet rs = stmt.executeQuery()) {
			while (rs.next()) {
				list.add(new PriceModel(rs.getInt("price_ID"), rs.getInt("price_monthly"),
						rs.getInt("price_deposit_amount")));
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	/**
	 * Deletes the room and any associated bookings to prevent foreign key
	 * constraint violations. Returns true if the room was successfully deleted,
	 * false otherwise.
	 */
	...
	public List<RoomModel> getFilteredRooms(String locationId, String priceId, String status) {
		...
		StringBuilder sql = new StringBuilder(
				"SELECT r.*, p.*, l.* FROM room r " + "JOIN price p ON r.price_ID = p.price_ID "
