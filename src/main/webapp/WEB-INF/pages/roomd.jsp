...
<body>
	...
	<section class="hero">
		<div class="hero-overlay">
			<h1><%=room.getRoom_type()%>
				in
				<%=room.getLocation_ID().getLocation_area()%></h1>
			<p>
				Comfortable and Convenient Living in
		...
		<div class="container">
			...
			<div class="gallery-grid">
				<img class="gallery-image"
					src="<%=request.getContextPath() + "/" + room.getImage_path()%>" />
	...
	<section class="room-details">
		<div class="container">
			...
			<div class="details-left">
				...
				<div class="section">
					<h2>Location</h2>
					<p><%=room.getLocation_ID().getLocation_area()%>,
						<%=room.getLocation_ID().getLocation_city()%></p>
				</div>
			</div>
			...
			<div class="details-right">
				<div class="pricing-card">
					...
					<p class="price">
						Rs
						<%=room.getPrice_ID().getPrice_monthly()%>
						/ month
					</p>
					<p
						class="status <%=room.getRoom_status().equalsIgnoreCase("Available") ? "available" : "unavailable"%>">
						<%=room.getRoom_status()%>
					</p>
					<p>
						<strong>Size:</strong>
					...
					<p>
						<strong>Deposit:</strong> Rs
						<%=room.getPrice_ID().getPrice_deposit_amount()%></p>
				</div>
							...
							<c:when test="${sessionScope.customer != null}">
								<form action="${pageContext.request.contextPath}/book-room"
									...
									<input type="hidden" name="room_ID"
										value="<%=room.getRoom_ID()%>" /> <input type="hidden"
										name="action" value="book" /> <label for="name">Full
										Name</label> <input type="text" id="name" name="name"
										value="<%=fullName%>" required /> <label for="email">Email</label>
									<input type="email" id="email" name="email" value="<%=email%>"
										required /> <label for="checkin">Check-in Date</label> <input
										type="date" id="checkin" name="checkin" required />

									<button type="submit" class="submit-button">Submit
										Request</button>

									<c:if test="${param.success == 'bookingSubmitted'}">
										<div class="alert alert-success">Booking request
											submitted successfully!</div>
									</c:if>
									<c:if test="${param.error == 'bookingFailed'}">
										<div class="alert alert-danger">Booking failed. Please
											try again.</div>
									</c:if>
									<c:if test="${param.error == 'alreadyBooked'}">
										<div class="alert alert-warning">You have already
											requested a booking for this room.</div>
									</c:if>
								</form>
							</c:when>
							<c:otherwise>
